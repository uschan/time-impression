...
import { TEXT_CONTENT } from '../../data/global';
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for(let i=0; i<points.length; i++) {
          ...
          if (distSq < MOUSE_RADIUS * MOUSE_RADIUS) {
              ...
              // Drag with mouse velocity (Fluid feel)
    ...
    const handleTouchMove = (e: TouchEvent) => {
        ...
        if(e.touches.length > 0) {
            const t = e.touches[0];
            const vx = t.clientX - mouseRef.current.lastX;
            const vy = t.clientY - mouseRef.current.lastY;
            mouseRef.current = { 
                x: t.clientX, 
                y: t.clientY, 
                vx: vx, 
